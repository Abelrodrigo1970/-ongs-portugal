...
export async function getEvents(filters = {}) {
  ...
  const where = {
    AND: [
      ...
      // Filtro por localização (busca em morada do evento ou localizacao da ONG)
      localizacao.length > 0 ? {
        OR: localizacao.flatMap(loc => [
          { morada: { contains: loc, mode: 'insensitive' } },
          { localizacao: { contains: loc, mode: 'insensitive' } },
          { ngo: { localizacao: { contains: loc, mode: 'insensitive' } } }
        ])
      } : {},
