...
export async function createInscricao(data) {
  ...
  try {
    ...
    // Erro de tabela não encontrada (P2021 ou erro de model não encontrado)
    if (error.message && error.message.includes('inscricao') && error.message.includes('undefined')) {
      throw new Error('A tabela "inscricoes" não existe no banco de dados. Por favor, execute o script SQL no Supabase: scripts/add-unique-inscricao-constraint.sql');
    }
    
    if (error.code === 'P2021') {
      throw new Error('A tabela "inscricoes" não foi encontrada. Execute o script SQL: scripts/add-unique-inscricao-constraint.sql no Supabase SQL Editor.');
    }
...
export async function updateInscricaoStatus(id, status) {
  try {
    ...
    console.error('Erro em updateInscricaoStatus:', error);
    throw error;
  }
}
...
export async function countInscricoesAprovadas(eventoId) {
  ...
  // Verificar se o model existe antes de tentar usar
  if (!prisma.inscricao) {
    console.warn('Tabela inscricoes não existe no banco. Execute o script SQL primeiro.');
    return 0;
  }
  ...
  try {
    ...
    // Se der erro (tabela não existe), retornar 0
    if (error.message && (error.message.includes('does not exist') || error.message.includes('inscricao'))) {
      console.warn('Tabela inscricoes não existe. Execute: scripts/add-unique-inscricao-constraint.sql no Supabase');
      return 0;
    }
    throw error;
  }
...
export async function getVagasEvento(eventoId) {
  ...
  // Tentar contar TODAS as inscrições (não apenas aprovadas), mas se der erro (tabela não existe), retornar 0
  let ocupadas = 0;
  try {
    ocupadas = await countInscricoes(eventoId); // Contar todas as inscrições
  } catch (error) {
    console.error('Erro ao contar inscrições (tabela pode não existir):', error.message);
    ocupadas = 0;
  }
