...
export async function getNGOs(filters = {}) {
  try {
    ...
    // Filtro por localização (suporta múltiplas localizações)
    if (localizacao.length > 0) {
      where.AND.push({
        OR: localizacao.map(loc => ({
          localizacao: {
            contains: loc,
            mode: 'insensitive'
          }
...
export async function createNGO(data) {
  try {
    const { ods = [], areas = [], colaboracao = [], projetos = [], impactos = [], ...ngoData } = data;
    ...
    const formattedImpacts = Array.isArray(impactos)
      ? impactos
          .slice(0, 3)
          .map((impact, index) => ({
...
export async function updateNGO(id, data) {
  try {
    ...
    if (Array.isArray(impactos)) {
      await prisma.nGOImpacto.deleteMany({ where: { ngoId: id } });
      const formattedImpacts = impactos
        .slice(0, 3)
        .map((impact, index) => ({
          valor: impact?.valor?.trim() || '',
          descricao: impact?.descricao?.trim() || '',
